...
3. **签名证书指纹/公钥已配置到 AGC**(账号、支付、推送类强校验项;换签名
   证书后必须同步更新,否则全线 401/校验失败——这是换机器/换证书后"突然
   全坏了"的标准答案)。
...
- token 会变化:监听刷新并重新上报,只取一次存数据库是掉推率的主因。
...
- 定位:优先 LocationButton 安全控件免权限(见 security-and-permissions);
  持续定位属长时任务场景。
- 扫码:Scan Kit 提供默认界面接口(一行调用)与自定义界面两档,先用默认档。
- 地图:AGC 开通 + 指纹配置后才出图;"地图白屏"基本是指纹/开通问题。
