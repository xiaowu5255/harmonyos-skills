...
当前版本：**0.7.4** — 8 plugin / 54 skill / 6 命令 / 2 hook / 全量质量审计 + 自进化工具链 + 信源质量防回潮 + 跨平台结构 + 主动纠错模式泛化 + 性能臆测软化 + Rust 鸿蒙集成 + evals 质量断言扩面。
...
- **防退化**：`tools/evals/evals.json` 121 条回归样本（56 条含质量断言：32 machine + 24 semantic，11 条负样本）；`tools/lint-skills.sh` 14 项一致性 + 内容质量检查（CI 强制）
...
- ✅ **121 条回归样本**：56 条含输出正确性断言（quality_assertion，32 machine 可机检 + 24 semantic 待人工），11 条负样本
