...
当前版本：**0.7.0** — 8 plugin / 53 skill / 6 命令 / 2 hook / 全量质量审计 + 自进化工具链 + 信源质量防回潮。
...
- **防退化**：`tools/evals/evals.json` 104 条回归样本（48 条含质量断言：24 machine + 24 semantic）；`tools/lint-skills.sh` 13 项一致性 + 内容质量检查（CI 强制）
...
- ✅ **13 项一致性检查**：`tools/lint-skills.sh` 覆盖 frontmatter 检查、路由表验证、references 引用完整性、JSON 解析、内容质量审查、8 高优 skill evals 覆盖软提示
- ✅ **104 条回归样本**：48 条含输出正确性断言（quality_assertion，24 machine 可机检 + 24 semantic 待人工），3 条负样本
