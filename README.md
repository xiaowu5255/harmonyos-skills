...
当前版本：**0.7.1** — 8 plugin / 53 skill / 6 命令 / 2 hook / 全量质量审计 + 自进化工具链 + 信源质量防回潮 + 跨平台结构 + 主动纠错模式泛化。
...
- **防退化**：`tools/evals/evals.json` 111 条回归样本（48 条含质量断言：24 machine + 24 semantic，10 条负样本）；`tools/lint-skills.sh` 14 项一致性 + 内容质量检查（CI 强制）
...
- ✅ **14 项一致性检查**：`tools/lint-skills.sh` 覆盖 frontmatter 检查、路由表验证、references 引用完整性、JSON 解析、内容质量审查、8 高优 skill evals 覆盖软提示、8 高优 skill 主动纠错覆盖率软提示
- ✅ **111 条回归样本**：48 条含输出正确性断言（quality_assertion，24 machine 可机检 + 24 semantic 待人工），10 条负样本
